...
try {
  ...
  const shellMain = await desktop.locator(".if-main.if-operations-app.if-operations-app--sticky-header").count();
  assert.equal(shellMain, 1, "app shell should mirror the OIP sticky operations app contract");
  ...
  assert.equal(utilityCluster, 0, "command header should not carry duplicate utility actions");
  const shellHeader = await desktop.locator("[data-fastdas-shell-header].if-product-header").count();
  assert.equal(shellHeader, 1, "header should expose the OIP product header contract");
  ...
  assert.equal(operationsTopnav, 3, "header should expose OIP-style primary surfaces through framework operations topnav links");
  const routeGroupTriggers = await desktop.locator("[data-fastdas-header-route] [data-nav-group-trigger]").count();
  assert.equal(routeGroupTriggers, 2, "header should expose two OIP-style dropdown route groups");
  ...
  assert.equal(visibleSecondaryItems, 3, "execution topnav menu should open from the header control");
  ...
  const profileSettings = await desktop.locator("[data-profile-menu-surface] [data-profile-setting]").count();
  assert.equal(profileSettings, 3, "profile menu should expose compact workspace-display rows");
  await desktop.keyboard.press("Escape");
  ...
  assert.equal(workingSetRibbon, 0, "command center should not render hidden working-set chrome");
  ...
  assert.equal(pageMeta, 0, "command center should not render page-header metadata");
  const operationsPage = await desktop.locator("[data-if-operations-workspace] .if-operations-page__topbar .if-breadcrumbs").count();
  assert.equal(operationsPage, 0, "command center should not render breadcrumb chrome");
  ...
  assert.equal(operationsHero, 0, "command center should not render page hero chrome");
  ...
  assert.equal(pageActions, 0, "command center should not render page action toolbar chrome");
  ...
  const commandStatusCounters = await desktop.locator("[data-fastdas-opportunity-grid] [data-if-table-status='filtered']").count();
  assert.equal(commandStatusCounters, 0, "command center table should not render a separate command-band counter");
  ...
  const commandBandAnatomy = await desktop.locator("[data-fastdas-opportunity-grid] .if-table-command-band__leading, [data-fastdas-opportunity-grid] .if-table-command-band__filters, [data-fastdas-opportunity-grid] .if-table-command-band__actions").count();
  assert.equal(commandBandAnatomy, 0, "command center table should not render an extra command band");
  const tableFilterControls = await desktop.locator("[data-fastdas-opportunity-grid] [data-if-table-filter]").count();
  assert.equal(tableFilterControls, 0, "command center should keep the table surface free of duplicate search controls");
  const tableStatusCounters = await desktop.locator("[data-fastdas-opportunity-grid] [data-if-table-status]").count();
  assert.equal(tableStatusCounters, 0, "command center should keep status counters out of the table chrome");
  ...
  await desktop.locator("[data-fastdas-header-route] .if-operations-topnav__link", { hasText: "Signal Intake" }).click();
  await desktop.waitForSelector("h1", { timeout: 5000 });
  assert.match(await desktop.locator("h1").textContent(), /Signal Intake/, "header nav should move operator to Signal Intake");
  ...
  assert.ok(opportunityFocusText.includes("HarborPoint Garage"), "opportunity workbench should start with its seeded demo record in focus");
  ...
  assert.ok(evidenceFocusText.includes("Fire alarm permit pattern"), "evidence review should start with its seeded demo evidence in focus");
  ...
  assert.ok(outreachFocusText.includes("HarborPoint Garage"), "outreach queue should start with its seeded demo task in focus");
  ...
  assert.ok(conversionFocusText.includes("Capital Ridge Senior Living"), "conversion board should start with its seeded demo record in focus");
  ...
  await desktop.locator('[data-fastdas-synthetic-input="signal"]').fill("Tenant cellular complaints + ownership change");
  await desktop.locator('[data-fastdas-synthetic-input="firstOffer"]').fill("DAS benchmark");
  await desktop.locator('[data-fastdas-synthetic-input="stakeholderPath"]').fill("Property manager -> owner rep");
  await desktop.locator('[data-fastdas-synthetic-input="score"]').fill("89");
  await desktop.locator('[data-fastdas-action="add-synthetic-record"]').click();
  await desktop.waitForSelector('[data-fastdas-table-row-id="Metro Center Garage"]');
  assert.equal(await desktop.locator("[data-fastdas-header-route]").getAttribute("data-fastdas-active-route"), "command-center", "adding a synthetic opportunity should move to Command Center");
  const syntheticCommandText = await desktop.locator('[data-fastdas-table-row-id="Metro Center Garage"]').textContent();
  assert.ok(syntheticCommandText.includes("DAS benchmark"), "entered synthetic opportunity should appear in the Command Center table");
  await desktop.goto(`${BASE_URL}#/synthetic-data`, { waitUntil: "domcontentloaded" });
  ...
  await desktop.waitForSelector('[data-fastdas-table-row-id="Metro Center Garage"]');
  const syntheticOutreachText = await desktop.locator('[data-fastdas-table-row-id="Metro Center Garage"]').textContent();
  ...
  assert.equal(await mobileMore.getAttribute("aria-expanded"), "false", "mobile More menu should start closed");
  ...
  assert.equal(mobileHeaderActions, 0, "mobile header should hide grouped actions in favor of working-set controls");
  const mobileProfileTrigger = mobile.locator("[data-profile-menu-trigger]");
  await mobileProfileTrigger.click();
  const mobileProfileSettings = await mobile.locator("[data-profile-menu-surface] [data-profile-setting]").count();
  assert.equal(mobileProfileSettings, 3, "mobile profile menu should keep workspace display controls");
  await mobile.keyboard.press("Escape");
  const mobileBreadcrumbs = await mobile.locator(".if-operations-page__topbar .if-breadcrumbs__current").count();
  assert.equal(mobileBreadcrumbs, 0, "mobile command center should not render breadcrumb chrome");
  const mobilePageMeta = await mobile.locator("[data-fastdas-page-meta] .if-route-status").count();
  assert.equal(mobilePageMeta, 0, "mobile command center should not render page meta chrome");
